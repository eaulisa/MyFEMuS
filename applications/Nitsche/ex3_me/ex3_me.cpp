...
void AssembleNitscheProblem_AD(MultiLevelProblem& ml_prob) {
  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    if(eFlag == 0 || eFlag == 2) {
      ...
      for(unsigned ig = 0; ig < msh->_finiteElement[ielGeom][solDType]->GetGaussPointNumber(); ig++) {
        ...
        // if dim=2 and nDofD = 4 then phi_x = [ phi0_x, phi0_y, phi1_x, phi1_y, phi2_x, phi2_y, phi3_x, phi3_y](x_gauss) so phi_x.size() =  dim*ndoD = 8
        ...
        for(unsigned i = 0; i < nDofD; i++) {
          for(unsigned k = 0; k < dim; k++) {
            for(unsigned j = 0; j < dim; j++) {
              gradSolD1[k][j] += phi_x[i * dim + j] * solD1[k][i]; // 2x2 gradSolD1 matrix interpolated with test functions evaluated at gauss point
        ...
        for(unsigned i = 0; i < nDofD; i++) {
          ...
          for(unsigned k = 0; k < dim; k++) {
            divD1 += gradSolD1[k][k] * (eFlag == 0); // this can be done using the idea above
 ...
 //first two are interface element contributions to the bulk terms
    else {
...
//       double theta = 2000. / den;
      ...
      while(imarker1 < markerOffset1[iproc + 1] && iel == particle1[imarker1]->GetMarkerElement()) {
        ...
        // the local coordinates of the particles are the Gauss points in this context. So we integrate at the particles.
        std::vector <double> xi = particle1[imarker1]->GetMarkerLocalCoordinates();
         // *** get test function and test function partial derivatives at markers ***
        msh->_finiteElement[ielGeom][solDType]->Jacobian(x, xi, weight, phi, phi_x); 
        // weights associated with the markers
