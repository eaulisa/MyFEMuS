...
public:
    ConicAdaptiveRefinement() {
        ...
        //TODO HIMALI:fail to reconize xr and yr according to the element type here
        /*      _xr.resize(_nve0[_data->_elType]);
    ...
    //TODO HIMALI: combined the the code for Quad and Tri by if(_data->_elType == 3) and else if(_data->_elType == 4)
    ...
    //HIMALI: a function to check the roots beween zero and one for Triangles
    bool CheckIfRootsAreInBetweenZeroandP1(const double & A, const double & B, const double & C) {
        double det = sqrt(B * B - 4. * A * C);
        if(det >= 0.) {
            double t = (-B - det) / (2. * A);
...
private:
    ...
    std::vector<unsigned>_nve0 = {0, 0, 0, 4, 3};
    std::vector<unsigned>_nve2 = {0, 0, 0, 9, 7};
    std::vector<std::vector<std::vector<unsigned>>> _idx = {
        {},
        {},
        {},
        {   // quad
...
//TODO: combined the the code for Quad and Tri by if(_data->_elType == 3) and else if(_data->_elType == 4)
...
int ConicAdaptiveRefinement::TestIfIntesectionWithReferenceQuad(const std::vector<double>&Ar) {
    ...
    if(_data->_elType == 3) {
        ...
        B = d;
        C = f;
        if(CheckIfRootsAreInBetweenZeroandP1(A, B, C)) return 0;
    ...
    if(level < levelMax) {
        ...
        if(test == 0) { // it means there is an intersection
            ...
            if(_data->_elType == 3) {
                ...
                x[6] = {(2 * x[0][0] + x[4][0]) / 3., (2 * x[0][1] + x[4][1]) / 3.};
                ...
                xil0[6] = {(2 * xil0[0][0] + xil0[4][0]) / 3., (2 * xil0[0][1] + xil0[4][1]) / 3.};
            }
