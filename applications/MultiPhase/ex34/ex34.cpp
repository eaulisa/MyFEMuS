...
template<> struct Geom<3> {
  static std::array<std::array<double,27>,3> coords(double a=-0.5,double b=+0.5){
    ...
    auto set=[&](int i,double xi,double eta,double z){ X[0][i]=xi; X[1][i]=eta; X[2][i]=z; };
...
template<> struct DimOps<2> {
  ...
  static BasisType highest_basis() { return BasisT<2>::Q9; }
  static void shift(Psi<2>& psi) { psi.xc = 0.0; psi.yc = 0.25; }
  static double evalPsi(const Psi<2>& psi, const Pt<2>& p) { return psi(p[0], p[1]); }
  ...
  static auto make_refine_pred(const PsiFunc& psi, double tau_refine, u32 /*cap*/) {
    return [&, tau_refine](u32, const std::vector<Pt<2>>&,
      ...
      for (size_t i=1;i<pts_xyz.size();++i){ double v=psi(pts_xyz[i][0],pts_xyz[i][1]); mn=std::min(mn,v); mx=std::max(mx,v); }
  ...
  static std::array<std::array<double,9>,2> geom() { return Geom<2>::coords(); }
  static double period() { return 8.0; }
};

template<> struct DimOps<3> {
  typedef BasisT<3> BasisType;
  static BasisType highest_basis() { return BasisT<3>::H27; }
  static void shift(Psi<3>& psi) { psi.xc = 0.0; psi.yc = 0.25; psi.zc = 0.0; }
  static double evalPsi(const Psi<3>& psi, const Pt<3>& p) { return psi(p[0], p[1], p[2]); }
  ...
  static auto make_refine_pred(const PsiFunc& psi, double tau_refine, u32 /*cap*/) {
    return [&, tau_refine](u32, const std::vector<Pt<3>>&,
      ...
      for (size_t i=1;i<pts_xyz.size();++i){ double v=psi(pts_xyz[i][0],pts_xyz[i][1],pts_xyz[i][2]); mn=std::min(mn,v); mx=std::max(mx,v); }
      return (mn<=0.0 && mx>=0.0) || (mx-mn > tau_refine);
    };
  }
  template<class PsiFunc>
  ...
  static std::array<std::array<double,27>,3> geom() { return Geom<3>::coords(); }
  static double period() { return 4.0; }
};
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "-d" || a == "--dim") {
      ...
      if (v <= 0) { std::cerr << "Error: --niter must be positive\n"; return 1; }
