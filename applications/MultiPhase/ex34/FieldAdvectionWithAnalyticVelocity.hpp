...
//----------------------------------------
  ...
  inline auto make_parent_vel_eval(
    ...
    return [&](const Point2& xiEta, double tau) {
      ...
      return eval_velocity_parent_analytic(qt, xiEta, t_abs, vfun);
    };
  }
...
//----------------------------------------
  template<class AnalyticVel>
  inline Point2 eval_velocity_parent_analytic(
    const QuadTree2D& qt,
    Point2 xi,
    double t_abs,
    AnalyticVel&& vfun) {
...
//----------------------------------------
  ...
  inline void advect_markers_forward_analytic(
    ...
    for (const auto& xiEta0 : coords0) {
      ...
      u32 leaf = qt0.locate_leaf_on_parent(xiEta1);
      if (leaf == npos32) {
        coordsLeftOld.push_back(qt0.parent_to_physical(xiEta0));
      }
      else {
        double N9[9];
        Shapes::Q9(xiEta1, N9);
...
//----------------------------------------
  ...
  inline void advect_nodes_backward_and_transport_field_analytic(
    ...
    // Destination field/basis and unique global nodes (no repetitions)
    Field& dst = qt1.field(fid1);
    ...
    for (size_t gid = 0; gid < nodes.size(); ++gid) {
      ...
      double val;
      const bool ok = qt0.evaluate_field_on_parent(fid0, xiEta0, val);
