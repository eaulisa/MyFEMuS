...
//----------------------------------------
  inline Point2 eval_velocity_parent(
    const QuadTree2D& qt,
    Basis velBasis,
    const std::vector<Point2>& vOld,
    const std::vector<Point2>& vNew,
    Point2 xi,
    double t, double dt) {
    // ------------------------------------
  ...
  inline void advect_markers_forward(
    ...
    for (auto& xiEta0 : coords0) {
      ...
      auto vel_eval = [&](const std::array<double, 2>& xiEta, double t) {
        return eval_velocity_parent(qt0, velBasis, vOld, vNew, xiEta, t, dt);
      };
      ...
      if (leaf == npos32) coordsLeftOld.push_back(qt0.parent_to_physical(xiEta0));
      else {
        double N9[9];
        Shapes::Q9(xiEta1, N9);
        double x1 = 0, y1 = 0;
...
// Free function (not a member)
  inline void advect_nodes_backward_and_transport_field(
    ...
    for (size_t gid = 0; gid < nodes.size(); ++gid) {
      ...
      (void) qt0.evaluate_field_on_parent(fid0, xiEta0, val);
