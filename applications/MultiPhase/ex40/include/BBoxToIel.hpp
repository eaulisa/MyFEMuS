...
class BBoxToIel {
public:
  BBoxToIel(MultiLevelMesh &mlMsh, const unsigned level,
            const unsigned nPartition)
      : _msh(mlMsh.GetLevel(level)), _dim(_msh->GetDimension()), _level(level),
        _iproc(_msh->processor_id()), _nprocs(_msh->n_processors()) {
    ...
    for (unsigned iel = offset; iel < offsetp1; ++iel) {
      ...
      for (unsigned solType = 0; solType < NSolType; ++solType) {
        ...
        ProjectNodalToPolynomialCoefficients(_aPall[iel - offset][solType],
                                             _xAll[iel - offset], ielType,
                                             solType);
      }
    }
  ...
  void Build(const unsigned nPartition) {
    ...
    for (unsigned iel = offset; iel < offsetp1; ++iel) {
      ...
      for (unsigned k = 0; k < _dim; ++k) {
        if (_xMin[_iproc][k] > xMinIel[k])
          _xMin[_iproc][k] = xMinIel[k];
        if (_xMax[_iproc][k] < xMaxIel[k])
          _xMax[_iproc][k] = xMaxIel[k];
    ...
    MPI_Allgather(_xMin[_iproc], _dim, MPI_DOUBLE, _xMinMemory.data(), _dim,
                  MPI_DOUBLE, MPI_COMM_WORLD);

    MPI_Allgather(_xMax[_iproc], _dim, MPI_DOUBLE, _xMaxMemory.data(), _dim,
                  MPI_DOUBLE, MPI_COMM_WORLD);
    ...
    for (unsigned k = 0; k < _dim; ++k) {
      if (hCellMin[k] <= 0.0) {
        throw std::runtime_error(
            "BBoxToIel::Build: nonpositive local element size");
      }
      ...
      _N[k] = static_cast<unsigned>(
          std::ceil((_xMax[_iproc][k] - _xMin[_iproc][k]) / _hBox[k]));
      if (_N[k] == 0u)
        _N[k] = 1u;

      _hBox[k] =
          (_xMax[_iproc][k] - _xMin[_iproc][k]) / static_cast<double>(_N[k]);
    }
    ...
    for (unsigned iel = offset; iel < offsetp1; ++iel) {
      ...
      for (unsigned k = 0; k < _dim; ++k) {
        int ikMin = static_cast<int>(
            std::floor((xMinIel[k] - _xMin[_iproc][k] - eps[k]) / _hBox[k]));
        int ikMax = static_cast<int>(
            std::floor((xMaxIel[k] - _xMin[_iproc][k] + eps[k]) / _hBox[k]));

        if (ikMin < 0)
          ikMin = 0;
        if (ikMax < 0)
          ikMax = 0;
        if (ikMin >= static_cast<int>(_N[k]))
          ikMin = static_cast<int>(_N[k]) - 1;
        if (ikMax >= static_cast<int>(_N[k]))
          ikMax = static_cast<int>(_N[k]) - 1;

        if (ikMax < ikMin)
          std::swap(ikMin, ikMax);
    ...
    for (unsigned ibox = 0; ibox < _bboxToIel.size(); ++ibox) {
      ...
      for (unsigned j = 0; j < _bboxToIel[ibox].size(); ++j) {
        ...
        GetClosestPointInReferenceElement(_xAll[jel - offset], xg, elType,
                                          _xi[ibox]);
        ok = GetInverseMapping_fast(2u, elType, _aPall[jel - offset], xg,
                                    _xi[ibox], 100u, _phi, _gradPhi);
        if (ok) {
  ...
  unsigned GetDim() const { return _dim; }

  const std::vector<double> &GetHBox() const { return _hBox; }

  const std::vector<unsigned> &GetN() const { return _N; }
  ...
  void GetInverseMappingOnCoarseLevel(std::vector<MyVector<double>> &X,
                                      LevelMarkers &lX, LevelMarkers &lY);
  void Project(const MultiLevelMesh &_mlMsh, LevelMarkers &lX,
               LevelMarkers &lY);
  ...
  // small helpers: avoid xi = {..} (which tends to show up as
  // vector::operator=)
  ...
  static inline void pushback_tet(double &x, double &y, double &z,
                                  const double eps) {
    if (x < 0.0 && x > -8.0 * eps)
      x = 0.0;
    if (y < 0.0 && y > -8.0 * eps)
  ...
  void GetChildIndexAndLocalXi(const unsigned et,
                               const std::vector<double> &xi0,
                               unsigned &childIndex, std::vector<double> &xi1,
                               const double eps = 1.0e-12) const;

  const unsigned &GetLevel() const { return _level; }

  void SetMesh(Mesh *msh) { _msh = msh; }

private:
  unsigned Flatten(const std::vector<unsigned> &idx) const {
    if (_dim == 1)
      return Flatten1D(idx[0]);
    if (_dim == 2)
      return Flatten2D(idx[0], idx[1]);
    if (_dim == 3)
      return Flatten3D(idx[0], idx[1], idx[2]);
  }

  unsigned Flatten1D(const unsigned i) const { return i; }

  unsigned Flatten2D(const unsigned i, const unsigned j) const {
    return i + _N[0] * j;
  }

  unsigned Flatten3D(const unsigned i, const unsigned j,
  ...
  void GetElementBoundingBox(const std::vector<std::vector<double>> &xv,
    ...
    if (xv.size() != _dim) {
      throw std::runtime_error(
          "BBoxToIel::GetElementBoundingBox: wrong dimension");
    }
    for (unsigned k = 0; k < _dim; ++k) {
      if (xv[k].empty()) {
        throw std::runtime_error(
            "BBoxToIel::GetElementBoundingBox: empty coordinate list");
      }
...
void BBoxToIel::GetInverseMappingOnCoarseLevel(std::vector<MyVector<double>> &X,
                                               LevelMarkers &lX,
                                               LevelMarkers &lY) {

  if (X.size() != _dim) {
    throw std::runtime_error(
        "GetInverseMappingOnCoarseLevel: X has wrong dimension");
  }
  ...
  for (unsigned k = 1; k < _dim; ++k) {
    if (X[k].size() != nPoints || X[k].begin() != X[0].begin() ||
        X[k].end() != X[0].end()) {
      throw std::runtime_error(
          "GetInverseMappingOnCoarseLevel: X[k] indexing is inconsistent");
    }
  }
  ...
  for (unsigned j = 0; j < _nprocs; j++)
    ZisInside_s[j].resize(map_s[j].size());
  ...
  // Exchange sizes
  MPI_Alltoall(size_s.data(), 1, MPI_UNSIGNED, size_r.data(), 1, MPI_UNSIGNED,
               MPI_COMM_WORLD);
  ...
  for (int j = 0; j < static_cast<int>(_nprocs); ++j) {
    if (j == static_cast<int>(_iproc)) {
      ...
      if (Z_r[j].size() > 0) {
        MPI_Request req;
        MPI_Irecv(Z_r[j].data(), static_cast<int>(Z_r[j].size()), MPI_DOUBLE, j,
  ...
  for (int j = 0; j < static_cast<int>(_nprocs); ++j) {
    if (j != _iproc) {
      if (Z_s[j].size() > 0) {
        MPI_Request req;
        MPI_Isend(Z_s[j].data(), static_cast<int>(Z_s[j].size()), MPI_DOUBLE, j,
                  100, MPI_COMM_WORLD, &req);
        reqs.push_back(req);
      }
    }
  ...
  for (unsigned jproc = 0; jproc < _nprocs; ++jproc) {
    if (Z_r[jproc].size() % _dim != 0) {
      throw std::runtime_error(
          "GetInverseMappingOnCoarseLevel: invalid packed receive size");
    }
    ...
    for (unsigned i = 0; i < Npoints; i++) {
      for (unsigned k = 0; k < _dim; k++) {
        ...
        int ik =
            static_cast<int>(std::floor((x[k] - _xMin[_iproc][k]) / _hBox[k]));
        if (ik < 0)
          ik = 0;
        if (ik >= static_cast<int>(_N[k]))
          ik = static_cast<int>(_N[k]) - 1;
      ...
      for (unsigned j = 0; j < _bboxToIel[ibox].size(); ++j) {
        ...
        if (j != 0 || _xi[ibox].size() == 0) {
          ...
          xi = _xi[ibox];
        bool ok = GetInverseMapping_fast(2u, elType, _aPall[jel - offset], x,
                                         xi, 100u, _phi, _gradPhi);
        if (ok) {
  ...
  for (int j = 0; j < static_cast<int>(_nprocs); ++j) {
    if (j == static_cast<int>(_iproc)) {
      ...
      if (ZisInside_s[j].size() > 0) {
        MPI_Request req;
        MPI_Irecv(ZisInside_s[j].data(),
                  static_cast<int>(ZisInside_s[j].size()), MPI_UNSIGNED, j, 100,
                  MPI_COMM_WORLD, &req);
        reqs.push_back(req);
      }
    }
  ...
  for (int j = 0; j < static_cast<int>(_nprocs); ++j) {
    if (j != _iproc && ZisInside_r[j].size() > 0) {
      MPI_Request req;
      MPI_Isend(ZisInside_r[j].data(), static_cast<int>(ZisInside_r[j].size()),
                MPI_UNSIGNED, j, 100, MPI_COMM_WORLD, &req);
      reqs.push_back(req);
    }
  }
  // Complete communication
  ...
  for (unsigned kproc = 0; kproc < _nprocs; ++kproc) {
    for (unsigned i = 0; i < ZisInside_s[kproc].size(); ++i) {
      ...
      ZisInside[idx] =
          ZisInside[idx] || static_cast<bool>(ZisInside_s[kproc][i]);
    }
  }

  for (unsigned kproc = 0; kproc < _nprocs; ++kproc) {
    for (unsigned i = 0; i < ZisInside_s[kproc].size(); ++i) {
      if (ZisInside_s[kproc][i] == 0)
        map_s[kproc][i] = uint_max;
    }
  }
...
void BBoxToIel::Project(const MultiLevelMesh &_mlMsh, LevelMarkers &lX,
                        LevelMarkers &lY) {
  ...
  if (levelX + 1 >= _mlMsh.GetNumberOfLevels()) {
    throw std::runtime_error(
        "MultiLevelMesh has not enough levels to projec the points");
  };
  ...
  for (unsigned kproc = 0; kproc < _nprocs; ++kproc) {
    ...
    if (size_r[kproc] > 0) {
      MPI_Request req1;
      MPI_Irecv(Yi_r[kproc].data(), static_cast<int>(Yi_r[kproc].size()),
                MPI_DOUBLE, static_cast<int>(kproc), tagYi, MPI_COMM_WORLD,
                &req1);
      requests.push_back(req1);

      MPI_Request req2;
      MPI_Irecv(YIel_r[kproc].data(), static_cast<int>(YIel_r[kproc].size()),
                MPI_UNSIGNED, static_cast<int>(kproc), tagYIel, MPI_COMM_WORLD,
                &req2);
  ...
  for (unsigned kproc = 0; kproc < _nprocs; ++kproc) {
    ...
    if (size_s[kproc] > 0) {
      MPI_Request req1;
      MPI_Isend(Yi_s[kproc].data(), static_cast<int>(Yi_s[kproc].size()),
                MPI_DOUBLE, static_cast<int>(kproc), tagYi, MPI_COMM_WORLD,
                &req1);
      requests.push_back(req1);

      MPI_Request req2;
      MPI_Isend(YIel_s[kproc].data(), static_cast<int>(YIel_s[kproc].size()),
                MPI_UNSIGNED, static_cast<int>(kproc), tagYIel, MPI_COMM_WORLD,
                &req2);
      requests.push_back(req2);
    }
...
void BBoxToIel::GetChildIndexAndLocalXi(const unsigned et,
  ...
  if (et == static_cast<unsigned>(LINE)) {
    if (xi0.size() != 1)
      throw std::runtime_error("project: Line3 expects xi size 1");
  ...
  if (et == static_cast<unsigned>(QUAD)) {
    if (xi0.size() != 2)
      throw std::runtime_error("project: Quad9 expects xi size 2");
    const double x = xi0[0];
    ...
    if (!top)
      childIndex = right ? 1u : 0u;
    else
      childIndex = right ? 2u : 3u;
  ...
  if (et == static_cast<unsigned>(HEX)) {
    if (xi0.size() != 3)
      throw std::runtime_error("project: Hex27 expects xi size 3");
    const double x = xi0[0];
    const double y = xi0[1];
  ...
  if (et == static_cast<unsigned>(TRI)) {
    ...
    if (xi0.size() != 2)
      throw std::runtime_error("project: Tri7 expects xi size 2");
    ...
    if (!inside_tri(r, s, eps)) {
      throw std::runtime_error(
          "project: Tri7 input xi not inside reference triangle");
    }
    ...
    if (xi1[0] < 0.0 && xi1[0] > -8.0 * eps)
      xi1[0] = 0.0;
    if (xi1[1] < 0.0 && xi1[1] > -8.0 * eps)
      xi1[1] = 0.0;
    ...
    if (!inside_tri(xi1[0], xi1[1], eps)) {
      ...
      std::cout << xi1[0] << " " << xi1[1] << std::endl;
      throw std::runtime_error(
          "project: Tri7 mapped xi not inside child reference triangle");
    }
#endif
    return;
  }
  ...
  if (et == static_cast<unsigned>(WEDGE)) {
    ...
    if (xi0.size() != 3)
      throw std::runtime_error("project: Wedge21 expects xi size 3");
    const double r = xi0[0];
    const double s = xi0[1];
    const double z = xi0[2];
    ...
    if (!inside_tri(r, s, eps) || !inside_line(z, eps)) {
      throw std::runtime_error(
          "project: Wedge21 input xi not inside reference wedge");
    }
    ...
    if (!inside_tri(xi1[0], xi1[1], eps) || !inside_line(xi1[2], eps)) {
      ...
      std::cout << "Wedge21 mapped : " << xi1[0] << " " << xi1[1] << " "
                << xi1[2] << "\n";
      throw std::runtime_error(
          "project: Wedge21 mapped xi not inside child reference wedge");
    }
#endif
    return;
  }
  ...
  if (et == static_cast<unsigned>(TET)) {
    ...
    if (xi0.size() != 3)
      throw std::runtime_error("project: Tet15 expects xi size 3");
    const double x = xi0[0];
    const double y = xi0[1];
    const double z = xi0[2];
    ...
    if (!inside_tet(x, y, z, eps)) {
      throw std::runtime_error(
          "project: Tet15 input xi not inside reference tet");
    }
    ...
    for (unsigned c = 0; c < 8u; ++c) {
      ...
      if (inside_tet(u, v, w, eps)) {
        ...
        if (!inside_tet(xi1[0], xi1[1], xi1[2], eps)) {
          ...
          std::cout << "Tet15 mapped : " << xi1[0] << " " << xi1[1] << " "
                    << xi1[2] << "\n";
          throw std::runtime_error(
              "project: Tet15 mapped xi not inside child reference tet");
        }
#endif
        return;
      }
    ...
    throw std::runtime_error(
        "project: Tet15 could not select a child (no candidate contained)");
  }
