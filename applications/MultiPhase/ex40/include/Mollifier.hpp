...
public:
  ...
  // ---------------- S4(u) = 126u^5 - 420u^6 + 540u^7 - 315u^8 + 70u^9 (u in
  // [0,1]) Expanded on s in [-1,1], s = dg1/eps: S4(s) = 1/2 + (315/256)s +
  // (-105/64)s^3 + (189/128)s^5 + (-45/64)s^7 + (35/256)s^9
  inline void SetConstantsC4(double eps) noexcept {
    ...
    // Coefficients are the expanded S4(s) coefficients, scaled by inv^k for use
    // in dg1^k
    _a0 = 0.5;
  ...
  inline double GetSmoothStepFunctionC4(double dg1) const noexcept {
    ...
    return _a0 +
           dg1 * (_a1 + dg2 * (_a3 + dg2 * (_a5 + dg2 * (_a7 + dg2 * _a9))));
  }

  // ---------------- C^2 (quintic) : zero value + first 2 derivatives at ±eps
  // ---------------- S2(u) = 6u^5 - 15u^4 + 10u^3 Expanded on s in [-1,1]:
  // S2(s) = 1/2 + (15/16)s + (-5/8)s^3 + (3/16)s^5
  ...
  // ---------------- S1(u) = 3u^2 - 2u^3 Expanded on s in [-1,1]: S1(s) = 1/2 +
  // (3/4)s + (-1/4)s^3
  ...
  // ---------------- S3(u) = 35u^4 - 84u^5 + 70u^6 - 20u^7 Expanded on s in
  // [-1,1]: S3(s) = 1/2 + (35/32)s + (-35/32)s^3 + (21/32)s^5 + (-5/32)s^7
