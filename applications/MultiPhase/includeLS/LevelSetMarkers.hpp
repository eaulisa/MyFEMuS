  ...
  public:
    ~LevelSetMarkers() {
      for(unsigned i = 0; i < 6; i++) {
        for(unsigned j = 0; j < 3; j++) {
          if( _refineElement[i][j] != NULL) {
            delete _refineElement[i][j];
          }
        }
      }
    }
    ...
    LevelSetMarkers(std::string name, uint dim, uint lmax1 = 5)
      ...
      if(dim == 3) {
        ...
        _refineElement[2][0] = new RefineElement(lmax1, "wedge", "linear", "third", "third", "legendre");
        _refineElement[2][1] = new RefineElement(lmax1, "wedge", "quadratic", "third", "third", "legendre");
        _refineElement[2][2] = new RefineElement(lmax1, "wedge", "biquadratic", "third", "third", "legendre");
      }
      else if (dim == 2) {
