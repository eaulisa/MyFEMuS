...
double EvaluateMu(MultiLevelSolution & mlSol) {
  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    double a = 0.; 
    boost::math::quaternion <double> b(0., 0., 0., 0.);
    boost::math::quaternion <double> f(0., 0., 0., 0.);
...
//       } // end phi_i loop
    ...
    double mu1 = f.R_component_1()/a;
    sol->_Sol[indexMu[0]]->set(iel, mu1);
    
    double mu2 = (f*conj(b)).R_component_1() / norm(b);
    sol->_Sol[indexMu[1]]->set(iel, mu2);
...
double EvaluateMuOld(MultiLevelSolution & mlSol) {
...
double EvaluateMuNormAverageAndElementPhase(MultiLevelSolution & mlSol) {
  ...
  double MuNormAverage = EvaluateMu(mlSol);
...
// *** Gauss point loop ***
    ...
    double numAll, denAll;

    MPI_Allreduce(&num, &numAll, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&den, &denAll, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

    double t = numAll / denAll;
    std::cout << t << "      ";

    double energy2AfterAll;
    MPI_Allreduce(&energy2After, &energy2AfterAll, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    std::cout << energy2AfterAll << " ";
  ...
  double energy2BeforeAll, energy2TAll;


  MPI_Allreduce(&energy2Before, &energy2BeforeAll, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(&energy2T, &energy2TAll, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

  std::cout << energy2BeforeAll << " " << " " << energy2TAll << std::endl;
  ...
  fout << counter << " " << energy2BeforeAll << " " << energy2TAll << std::endl;
  fout.close();
