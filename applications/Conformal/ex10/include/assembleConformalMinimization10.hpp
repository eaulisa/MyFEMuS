...
void AssembleConformalMinimization(MultiLevelProblem& ml_prob) {
  ...
  // Get Lambda info
  std::vector < unsigned > solLIndex(DIM);
  unsigned solLType;
  std::vector < unsigned > solLPdeIndex(DIM);
  std::vector < std::vector < double > > solL(DIM);
  if(parameter.constraintIsOn) {
    solLIndex[0] = mlSol->GetIndex("L1");
    solLIndex[1] = mlSol->GetIndex("L2");
    solLIndex[2] = mlSol->GetIndex("L3");
    solLType = mlSol->GetSolutionType(solLIndex[0]);
    solLPdeIndex[0] = mlPdeSys->GetSolPdeIndex("L1");
    solLPdeIndex[1] = mlPdeSys->GetSolPdeIndex("L2");
    solLPdeIndex[2] = mlPdeSys->GetSolPdeIndex("L3");
  }
  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    // Resize local arrays
    unsigned sizeAll = DIM * nxDofs + DIM * nLDofs + areaConstraint;
    ...
    if(parameter.constraintIsOn) {
      for(unsigned i = 0; i < nLDofs; i++) {
        ...
        for(unsigned K = 0; K < DIM; K++) {
          ...
          SYSDOF[DIM * nxDofs + K * nLDofs + i] = pdeSys->GetSystemDof(solLIndex[K], solLPdeIndex[K], i, iel);
        }
      }
    }
    ...
    for(unsigned ig = 0; ig < msh->_finiteElement[ielGeom][solType]->GetGaussPointNumber(); ig++) {
      ...
      for(unsigned I = 0; I < DIM; I++) {
        for(unsigned i = 0; i < nxDofs; i++) {
          ...
          unsigned irow1 = DIM * nxDofs + I * nLDofs + i;
          unsigned istart1 = irow1 * sizeAll;
          for(unsigned J = 0; J < DIM; J++) {
            for(unsigned j = 0; j < nxDofs; j++) {
              ...
              for(unsigned k = 0; k < dim; k++) {
                for(unsigned l = 0; l < dim; l++) {
                  ...
                  term1 -= phix_uv[k][i] * gi[k][l] * phix_uv[l][j];
                }
              }
              Jac[istart + J * nxDofs + j] += term * Area;
              Jac[istart + DIM * nxDofs + J * nLDofs + j] += term1 * Area;

              Jac[istart1 + J * nxDofs + j] += term1 * Area;


              Res[irow] -= term  * Area * (xhat[J][j] + solDx[J][j]) + term1  * Area * (solL[J][j]) ;
              Res[irow1] -= term1  * Area * (xhat[J][j] + solDx[J][j]) ;
            }
          }
        }
      }
        ...
        for(unsigned i = 0; i < nLDofs; i++) {
          ...
          for(unsigned J = 0; J < DIM; J++) {
            for(unsigned j = 0; j < nLDofs; j++) {
              Res[irow1] -= phiL[i] * (-0.001 * eps * solL[J][j] * Area);
              //Jac[istart1 + DIM * nxDofs + J * nLDofs + j] += -100 * phiL[i] * phiL[j] *  Area;
              Jac[istart1 + irow1] += -0.001 * eps * phiL[i] * phiL[j] *  Area;
            }
          }
        }
      }
