...
void GetNormalTetBF(const std::vector < std::vector<double> > &xv, const std::vector<double> &xg, const double & R, std::vector<double> &a, double & d7,  std::vector<double> &xm, std::vector<double> &a2, double & d2, double & volume,  unsigned & cut) {
  ...
  if(cnt == 0) {
    ...
    double t0 = 0.5 * (M_PI - 2. * M_PI / cnt);
    double t = 2. * asin(sin(t1) * cos(t0));
    double t2 = t * t;
    double t4 = t2 * t2;
    ...
    double ft = AA[cnt][0] * t4 + AA[cnt][1] * t2 + AA[cnt][2];
    double H = R * cos(t1);
    double R1 = H + ft * (R - H);
    ...
    xm.resize(dim);
    xm[0] = R1 * sin(phi) * cos(theta);
    xm[1] = R1 * sin(phi) * sin(theta);
    xm[2] = R1 * cos(phi);
...
void GetNormalHexBF1(const std::vector < std::vector<double> > &xv, const std::vector<double> &xg, const double & R, std::vector<double> &a, double & d,  std::vector<double> &xm, std::vector<double> &a2, double & d2, double & volume,  unsigned & cut, const elem_type *fem) {
  ...
  if(cnt == 0) {
    ...
    std::cout << "bad: small ball, number of intersection in hex = " << cnt << std::endl;
    cut = 0;
    return;
  }
    ...
    xm.resize(dim);
    xm[0] = R1 * sin(phi) * cos(theta) + xg[0];
    xm[1] = R1 * sin(phi) * sin(theta) + xg[1];
    xm[2] = R1 * cos(phi) + xg[2];
