...
void AssembleConformalMinimization(MultiLevelProblem& ml_prob) {
  ...
  solDxIndex[0] = mlSol->GetIndex("nDx1");
  solDxIndex[1] = mlSol->GetIndex("nDx2");
  solDxIndex[2] = mlSol->GetIndex("nDx3");

  std::vector < unsigned > solDxOldIndex(DIM);
  solDxOldIndex[0] = mlSol->GetIndex("Dx1");
  solDxOldIndex[1] = mlSol->GetIndex("Dx2");
  solDxOldIndex[2] = mlSol->GetIndex("Dx3");

  unsigned solType = mlSol->GetSolutionType(solDxIndex[0]);
  std::vector < unsigned > solDxPdeIndex(DIM);
  solDxPdeIndex[0] = mlPdeSys->GetSolPdeIndex("nDx1");
  solDxPdeIndex[1] = mlPdeSys->GetSolPdeIndex("nDx2");
  solDxPdeIndex[2] = mlPdeSys->GetSolPdeIndex("nDx3");
  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    for(unsigned i = 0; i < nxDofs; i++) {
      ...
      for(unsigned K = 0; K < DIM; K++) {
        xhat[K][i] = (*msh->_topology->_Sol[K])(iXDof) + (*sol->_Sol[solDxOldIndex[K]])(iDDof);
        solDx[K][i] = (*sol->_Sol[solDxIndex[K]])(iDDof) - (*sol->_Sol[solDxOldIndex[K]])(iDDof);
