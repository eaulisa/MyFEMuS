...
bool SetBoundaryCondition(const std::vector < double >& x, const char SolName[], double& value, const int facename, const double time) {
  ...
  if(!strcmp("W1", SolName) || !strcmp("W2", SolName) || !strcmp("W3", SolName)) {
    dirichlet = false;
  }
        ...
        for(unsigned ig = 0; ig  <  msh->_finiteElement[faceGeom][solxType]->GetGaussPointNumber(); ig++) {
          ...
          for(unsigned I = 0; I < DIM; I++) {
            for(unsigned i = 0; i < nxDofs; i++) {
              ...
              solxg[I] += phi[i] * solx[I][inode].value();
              solxg_u[I] += phi_u[i] * solx[I][inode].value();
            }
          }
          ...
          for(unsigned i = 0; i < nxDofs; i++) {
            for(unsigned I = 0; I < DIM; I++) {
              ...
              aResW[I][idof] += -solxg[I] * phi[i] * length;
            }
          }
        }
      }
    ...
    for (unsigned ig = 0; ig < msh->_finiteElement[ielGeom][solxType]->GetGaussPointNumber(); ig++) {
      ...
      for (unsigned K = 0; K < DIM; K++) {
        for (unsigned i = 0; i < nxDofs; i++) {
          ...
          aResW[K][i] += (solYg[K] * phix[i] + term1) * Area; // delta 1 smooth things out (stability trick) to be used for bad surfaces,
        }
        ...
        for (unsigned i = 0; i < nWDofs; i++) {
          ...
          // P-Willmore equation
          aResx[K][i] += ( ( (solLambda1 /*- YdotN * solLambda2*/) * normal[K] + (solxNewg[K] - solxOldg[K])  / dt) * phiW[i]
                           + solLambda2 * term1
                           - term0
                           + sumP2 * term1
                           - term2 * phiW_Xtan[K][i]
