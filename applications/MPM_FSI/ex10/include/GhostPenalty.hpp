  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    if(eFlag1 == 1) {
      ...
      for(unsigned iface = 0; iface < msh->GetElementFaceNumber(iel); iface++) {
        ...
        unsigned jproc = (jel >= 0) ? msh->IsdomBisectionSearch(jel , 3) : nprocs; // return  jproc for piece-wise constant discontinuous type (3)
        if(jproc == iproc) {
      ...
      for(int iel = msh->_elementOffset[kproc]; iel < msh->_elementOffset[kproc + 1]; iel++) {
        ...
        if(eFlag1 == 1) {
          ...
          for(unsigned iface = 0; iface < nFaces; iface++) {
            ...
            if(jel >= 0) { // if iface is not a boundary of the domain
              ...
              if(jproc != kproc && (iproc == kproc || iproc == jproc) ) {
                ...
                unsigned eFlag2;
                if(iproc == jproc) {
                  eFlag2 = (jel >= 0) ? static_cast <unsigned>(floor((*mysolution->_Sol[eflagIndex])(jel) + 0.5)) : 3;
                ...
                if(eFlag2 == 0 + !fluid * 2 || (eFlag2 == 1 && jel > iel)) {
                  ...
                  if(iproc == kproc) {
                    ...
                    MPI_Send(&ielt1, 1, MPI_UNSIGNED, jproc, 0, PETSC_COMM_WORLD);
                    ...
                    MPI_Recv(&ielt1, 1, MPI_UNSIGNED, kproc, 0, PETSC_COMM_WORLD, MPI_STATUS_IGNORE);
                  }

                  unsigned nDofsV1 = el->GetNVE(ielt1, solTypeV);
                  unsigned nDofsP1 = el->GetNVE(ielt1, solTypeP);
                  unsigned nDofs1 = dim * nDofsV1 + nDofsP1;
                  ...
                  //std::vector < MPI_Request > reqsRecv(3 * dim + 2);
                  ...
                  if(iproc == kproc) {
                    ...
                    for(unsigned k = 0; k < dim; k++) {
                      MPI_Irecv(solV2[k].data(), solV2[k].size(), MPI_DOUBLE, jproc, k, PETSC_COMM_WORLD, &reqs[k]);
                      //MPI_Isend(solV1[k].data(), solV1[k].size(), MPI_DOUBLE, jproc, k, PETSC_COMM_WORLD, &reqsSend[k]);

                      MPI_Irecv(solV2Old[k].data(), solV2Old[k].size(), MPI_DOUBLE, jproc, k + dim, PETSC_COMM_WORLD, &reqs[k + dim]);
                      //MPI_Isend(solV1Old[k].data(), solV1Old[k].size(), MPI_DOUBLE, jproc, k + dim, PETSC_COMM_WORLD, &reqsSend[k + dim]);

                      MPI_Irecv(vx2[k].data(), vx2[k].size(), MPI_DOUBLE, jproc, k + 2 * dim, PETSC_COMM_WORLD, &reqs[k + 2 * dim]);
                      //MPI_Isend(vx1[k].data(), vx1[k].size(), MPI_DOUBLE, jproc, k + 2 * dim, PETSC_COMM_WORLD, &reqsSend[k + 2 * dim]);
                    }

                    MPI_Irecv(solP2.data(), solP2.size(), MPI_DOUBLE, jproc, 3 * dim, PETSC_COMM_WORLD,  &reqs[3 * dim]);
                    //MPI_Isend(solP1.data(), solP1.size(), MPI_DOUBLE, jproc, 3 * dim, PETSC_COMM_WORLD,  &reqsSend[3 * dim]);

                    MPI_Irecv(sysDofs2.data(), sysDofs2.size(), MPI_UNSIGNED, jproc, 3 * dim + 1, PETSC_COMM_WORLD,  &reqs[3 * dim + 1]);
                    //MPI_Isend(sysDofs1.data(), sysDofs1.size(), MPI_UNSIGNED, jproc, 3 * dim + 1, PETSC_COMM_WORLD,  &reqsSend[3 * dim + 1]);
                    ...
                    for(unsigned k = 0; k < dim; k++) {
                      //MPI_Irecv(solV1[k].data(), solV1[k].size(), MPI_DOUBLE, kproc, k, PETSC_COMM_WORLD, &reqsRecv[k]);
                      MPI_Isend(solV2[k].data(), solV2[k].size(), MPI_DOUBLE, kproc, k, PETSC_COMM_WORLD, &reqs[k]);

                      //MPI_Irecv(solV1Old[k].data(), solV1Old[k].size(), MPI_DOUBLE, kproc, k + dim, PETSC_COMM_WORLD, &reqsRecv[k + dim]);
                      MPI_Isend(solV2Old[k].data(), solV2Old[k].size(), MPI_DOUBLE, kproc, k + dim, PETSC_COMM_WORLD, &reqs[k + dim]);

                      //MPI_Irecv(vx1[k].data(), vx1[k].size(), MPI_DOUBLE, kproc, k + 2 * dim, PETSC_COMM_WORLD, &reqsRecv[k + 2 * dim]);
                      MPI_Isend(vx2[k].data(), vx2[k].size(), MPI_DOUBLE, kproc, k + 2 * dim, PETSC_COMM_WORLD, &reqs[k + 2 * dim]);
                    }

                    //MPI_Irecv(solP1.data(), solP1.size(), MPI_DOUBLE, kproc, 3 * dim, PETSC_COMM_WORLD, &reqsRecv[3 * dim]);
                    MPI_Isend(solP2.data(), solP2.size(), MPI_DOUBLE, kproc, 3 * dim, PETSC_COMM_WORLD, &reqs[3 * dim]);

                    //MPI_Irecv(sysDofs1.data(), sysDofs1.size(), MPI_UNSIGNED, kproc, 3 * dim + 1, PETSC_COMM_WORLD, &reqsRecv[3 * dim + 1]);
                  ...
                  if(iproc == kproc) {
                    ...
                    for(unsigned ig = 0; ig  <  msh->_finiteElement[faceGeom][solTypeV]->GetGaussPointNumber(); ig++) {
                      ...
                      short int unsigned sielt1 = ielt1;
                      bool inverseMapping = GetInverseMapping(solTypeV, sielt1, aP1, xg, xi1, 100);
                      if(!inverseMapping) {
                        std::cout << "InverseMapping1 failed at " << iel << " " << jel << " " << iface << std::endl;
                      }
                      ...
                      short int unsigned sielt2 = ielt2;
                      inverseMapping = GetInverseMapping(solTypeV, sielt2, aP2, xg, xi2, 100);
                      if(!inverseMapping) {
                        std::cout << "InverseMapping2 failed at " << iel << " " << jel << " " << iface << std::endl;
                      }
