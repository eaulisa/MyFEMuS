  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    for(unsigned i = 0; i < nDofs; i++) {
      ...
      for(unsigned  k = 0; k < dim; k++) {
        ...
        vx[k][i]  = vxHat[k][i] + solDTld[k][i]; // deformed configuration at alpha_f/theta
    ...
    for(unsigned ig = 0; ig < msh->_finiteElement[ielt][solType]->GetGaussPointNumber(); ig++) {
      ...
      for(unsigned i = 0; i < nDofs; i++) {
        ...
        for(unsigned j = 0; j < dim; j++) {
          ...
          solVgTheta[j] += phi[i] * solV[j][i] ; //velocity in the deformed theta configuration
          ...
          for(unsigned  k = 0; k < dim; k++) {
            ...
            gradSolVgTheta[k][j] += gradPhi[i * dim + j] * solV[k][i] ; // gradient of the theta velocity with respect to the theta domain
          }
        }

        for(unsigned j = 0; j < dim2; j++) {
          for(unsigned  k = 0; k < dim; k++) {
            DeltaSolVgTheta[k][j]   += nablaphi[i * dim2 + j] * solV[k][i]  ; // laplace of the theta velocity with respect to the theta domain
          }
        }
      }
    ...
    if(eFlag > 0) {   // interface and solid
      ...
      while(iBmarker < markerOffsetBulk[iproc + 1] && iel == particlesBulk[iBmarker]->GetMarkerElement()) {
        ...
        for(int j = 0; j < dim; j++) {
          for(unsigned i = 0; i < nDofs; i++) {
            ...
            for(int k = 0; k < dim; k++) {
              ...
              gradSolVpTheta[j][k] +=  gradPhi[i * dim + k] * solV[j][i];
              gradSolDpHat[k][j] +=  solDTld[k][i] * gradPhiHat[i * dim + j];
        ...
        for(int j = 0; j < dim; j++) {
          solVpTheta[j] = solVp[j]; //TODO this comes from the particle, try the old mesh velocity
      ...
      if(eFlag == 1) {  //interface markers
        ...
        while(imarkerI < markerOffsetI[iproc + 1] && iel == particleI[imarkerI]->GetMarkerElement()) {
          ...
          for(int k = 0; k < dim; k++) {
            ...
            v2[k] = solVpOld[k] +  dt * (Gamma * solAp[k] + (1. - Gamma) * solApOld[k]);
          }
          ...
          for(unsigned k = 0; k < dim; k++) {
            ...
            for(unsigned i = 0; i < nDofs; i++) {
              for(unsigned j = 0; j < dim; j++) {
                tau[k] += -muFluid * (solV[k][i] * gradPhi[i * dim + j] +
                                      solV[j][i] * gradPhi[i * dim + k]) * N[j];
              }
            }
          }
          ...
          double thetaL = GAMMA * rhoFluid * ((c / 6.) + h / (12. * 1. * dtMin)) + thetaM;  // [rho L/T]
