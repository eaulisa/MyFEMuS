...
//               aResD[k][i] +=  wlaplaceD * weightHat;
    ...
    if(eFlag > 0) {   // interface and solid
      ...
      while(iBmarker < markerOffsetBulk[iproc + 1] && iel == particlesBulk[iBmarker]->GetMarkerElement()) {
        ...
        std::vector <std::vector <adept::adouble> > JacI;
        adept::adouble detJ;
        msh->_finiteElement[ielt][solType]->GetJacobianMatrix(vx, xi, detJ, Jac, JacI); //centered at theta
...
//                                Line & solidLine, Line & fluidLine, Line & interfaceLine,
  ...
  vector < vector< vector < double > > > gradSolDHat(dim);
  ...
  for(unsigned iMarker = offset0; iMarker < markerOffset[iproc + 1]; iMarker++) {
    ...
    if(iel != UINT_MAX) {
      ...
      if(iel != ielOld) {
        ...
        for(unsigned inode = 0; inode < nDofs; inode++) {
          ...
          for(int i = 0; i < dim; i++) {
            ...
            for(int k = 0; k < dim; k++) {
              gradSolDHat[i][k][inode] = (*mysolution->_Sol[indexGradSolD[i][k]])(idof);;
            }
          }
      ...
      for(int i = 0; i < dim; i++) {
        ...
        for(int j = 0; j < dim; j++) {
          for(unsigned inode = 0; inode < nDofs; inode++) {
            gradSolDgHat[i][j] +=  phiHat[inode] * gradSolDHat[i][j][inode];
          }
        }
      }
      std::vector < std::vector < double > > FpOld;
