 ...
 * the following function assembles (explicitely) the Jacobian matrix J(u^i) and the residual std::vector Res(u^i)
    ...
    for(unsigned ig = 0; ig < msh->_finiteElement[ielGeom][soluType]->GetGaussPointNumber(); ig++) {
      ...
      for(unsigned i = 0; i < nDofs; i++) {
        ...
        for(unsigned k = 0; k < dim; k++) {
          ...
          nonLinearTerm += soluGauss * soluGauss_x[k] * phi[i];
        }
...
void AssembleNonlinearProblem_AD(MultiLevelProblem& ml_prob) {
  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    mRes.resize(nDofs);    //resize
    std::fill(mRes.begin(), mRes.end(), 0);    //set mRes to zero
