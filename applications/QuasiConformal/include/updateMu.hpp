...
double EvaluateMu(MultiLevelSolution & mlSol) {
  ...
  indexDx[0] = mlSol.GetIndex("nDx1");
  indexDx[1] = mlSol.GetIndex("nDx2");
  indexDx[2] = mlSol.GetIndex("nDx3");
  
  std::vector < unsigned > indexDxOld(DIM);
  indexDxOld[0] = mlSol.GetIndex("Dx1");
  indexDxOld[1] = mlSol.GetIndex("Dx2");
  indexDxOld[2] = mlSol.GetIndex("Dx3");
  ...
  for(int iel = msh->_elementOffset[iproc]; iel < msh->_elementOffset[iproc + 1]; iel++) {
    ...
    for(unsigned i = 0; i < nDofsDx; i++) {
      ...
      for(unsigned K = 0; K < DIM; K++) {
        xhat[K][i] = (*msh->_topology->_Sol[K])(xDof) + (*sol->_Sol[indexDxOld[K]])(idof);
