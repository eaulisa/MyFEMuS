...
//  }
    ...
    if (_restart_flag) {
      ...
      std::cout << "Normal simulation without restart" << std::endl;
...
//      if (status2 != 0) { std::cout << "No problem for now" << std::endl;}
        ...
        if (status != 0) {
          std::cout << "MKDIR error: " << status << std::endl;
          abort();
        }
...
//this is one reason why a function that was born for the shell is not so well done in C
        ...
        std::cout << "Some conflicting run at exactly the same time, unlikely but check it" << std::endl;
        abort();
      }
