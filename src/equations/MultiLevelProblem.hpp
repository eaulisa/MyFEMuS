...
//   void init();
      ...
      inline system_iterator       begin()       {
        return _systems.begin();
      }

      /**  */
      inline system_iterator         end()       {
        return _systems.end();
      }

      /**  */
      inline const_system_iterator begin() const {
        return _systems.begin();
      }

      /**  */
      inline const_system_iterator   end() const {
        return _systems.end();
      }

      /**  New get/set for new data */
      inline void SetMeshTwo(const MultiLevelMeshTwo * mesh_in)  {
        _mesh = mesh_in;
        return;
      }

      inline const  MultiLevelMeshTwo & GetMeshTwo() const {
        return  *_mesh;
      }

      /** Quantity Map */
      inline void SetQtyMap(const QuantityMap * qtymap_in) {
        _qtymap = qtymap_in;
        return;
      }

      inline const QuantityMap & GetQtyMap() const {
        return  *_qtymap;
      }

      /** ElemType and Quadrature rule */
      inline const std::vector<const elem_type*>  & GetElemType(const unsigned dim) const {
        return  _elem_type[dim - 1];
      }

      inline const std::vector< std::vector<const elem_type*> >  & GetElemType() const {
        return  _elem_type;
      }

      inline const std::vector<Gauss> & GetQuadratureRuleAllGeomElems() const {
        return _qrule;
      }

      inline const Gauss & GetQuadratureRule(const unsigned geom_elem_type) const {
        return _qrule[geom_elem_type];
      }

      void SetQuadratureRuleAllGeomElems(const std::string quadr_order_in);

      /** Input Parser */
      inline const FemusInputParser<double> &  GetInputParser() const {
        return *_phys;
      }

      void SetInputParser(const FemusInputParser<double> * parser_in) {
        _phys = parser_in;
        return;
      }

      /** Files Handler */
      void SetFilesHandler(const Files * files_in) {
        _files = files_in;
        return;
      }

      inline const Files * GetFilesHandler() const {
        return  _files;
      }

      /** Input Parser */
      const int get_current_system_number() const {
        return _current_system_number;
      }

      void set_current_system_number(const unsigned current_system_number_in) {
        _current_system_number = current_system_number_in;
      }
      ...
      std::map<std::string, System*> & get_systems_map() {
        return _systems;
      }

      void get_all_abstract_fe(std::vector < std::vector < /*const*/ elem_type_templ_base< double, double > *  > > & elem_all_in) {               /*const*/
        elem_all_in = _elem_all_dd;
      }

      void get_all_abstract_fe(std::vector < std::vector < /*const*/ elem_type_templ_base< adept::adouble, double > *  > > & elem_all_in) {       /*const*/
        elem_all_in = _elem_all_ad;
      }

      void get_all_abstract_fe(std::vector < std::vector < /*const*/ elem_type_templ_base< adept::adouble, adept::adouble > *  > > & elem_all_in) { /*const*/
        elem_all_in = _elem_all_aa;
      }
...
//    std::cout << " FE Evals time " << static_cast<double>(end_evals - start_evals) / CLOCKS_PER_SEC << std::endl;
      ...
      void SetMultiphaseParams(MultiLevelProblem* mlProbF,
                               ...
                               const unsigned level0, const double hC,
                               MultiphasePhysicalProperties properties, TimeDiscretization td) {
        ...
        _multiphaseParams.hC = hC;
        _multiphaseParams.properties = properties;
        _multiphaseParams.td = td;
